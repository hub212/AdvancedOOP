...
//calculates the canter of the game board
...
//uses a predetarmined function to assign location on board to a value
...
//calculates the smalles deiiference between two values the function from mapCellToValue gets
...
//maps each location on the board to a distinct value such that the location in the center of the board get higher values then the edges of the board
...
//maps each location on the board to a distinct value in a random mannder
...
double Smart3DPlayer::rankMove(Coordinate move) const
{
	...
	double cellIndex = getLocationIndex(move);
...
//lowers the score for a location that can't hold the enemy remaining ships (based on their persumed size)
...
//increases score for locations based on the number of remaining enemy ships they can hold
...
//counts the number of ship up to a given size
...
//generates a random index for rankedLegalAttackMoves
int Smart3DPlayer::getRandMoveIndex() const{
	int firstValidMoveLocation = getFirstValidMoveLocation();
	if (firstValidMoveLocation == -1)
		firstValidMoveLocation = 0;
	std::uniform_int_distribution<std::mt19937::result_type> attackDist(firstValidMoveLocation, rankedLegalAttackMoves.size());

	return attackDist(rng);
}

//gets the first location with a value equal or higher then 1
int Smart3DPlayer::getFirstValidMoveLocation() const {
	int index = 0;
	int location = -1;

	for (auto p : rankedLegalAttackMoves)
	{
		if (p.first >= 1)
			return index;
		index++;
...
void Smart3DPlayer::notifyOnAttackResult(int player, Coordinate move, AttackResult result)
{
	...
	switch (result)
	{
		case AttackResult::Miss:
			if (player == playerID)//document miss
				consecutiveMisses++;
			break;
...
//updates numOfRows, numOfCols, boardDepth and allocate the board. Also counts the players' ships by type
...
//counts the ship type cur represents
...
// required to use unordered_set with Coordinate
bool operator==(const Coordinate& c1, const Coordinate& c2) {
