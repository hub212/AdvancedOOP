...
class Smart3DPlayer : public IBattleshipGameAlgo {
	...
	vector<int> shipTypes{ 0,0,0,0 }; //documents the number of every ship size on the board
	int smallestEnemyShipIndex = 0;//index in shipTypes for the smallest remaining enemy ship
	map<Coordinate, int> locationToIndexMap;//holds unique index between 0 to numOfRows*numOfCols*boardDepth-1 for every location on board
	...
	int consecutiveMisses=0;//counts consecutive misses of the player
	...
	void createGameBoard(const BoardData& board);//updates numOfRows, numOfCols, boardDepth and allocate the board. Also counts the players' ships by type
	...
	void updateShipCount(char cur);//counts the ship type cur represents
	int calcNumOfShipsUpToGivenSize(int size) const;//counts the number of ship up to a given size
	double calcMitigatedScoreForUnlikelyLocation(double curScore) const;//lowers the score for a location that can't hold the enemy remaining ships (based on their persumed size)
	double factorScoreForLikelyLocation(double curScore, int maxShipSize) const;//increases score for locations based on the number of remaining enemy ships they can hold
	Coordinate calc3DCubeCenter() const; //calculates the canter of the game board
	double mapCellToValue(Coordinate point) const;//uses a predetarmined function to assign location on board to a value
	double calcFuncMinDiff() const;//calculates the smalles deiiference between two values the function from mapCellToValue gets
	void createCenteredCellToIndexMapping();//maps each location on the board to a distinct value such that the location in the center of the board get higher values then the edges of the board
	void savePerviousGameInfo();//saves results of index celection stratagies of previous game
	void createRandomCellToIndexMapping();//maps each location on the board to a distinct value in a random mannder
	void createCellToIndexMapping();//maps each location on the board to a distinct value - can choose between two different index allocation strategies and chooses based on past successes of each strategy.
	int getRandMoveIndex() const;//generates a random index for rankedLegalAttackMoves
	int getFirstValidMoveLocation() const;//gets the first location with a value equal or higher then 1
